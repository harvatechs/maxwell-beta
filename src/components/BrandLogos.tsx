...
/**
 * Official MaxWell Favicon Mark
 * Rendered using the root favicon.png image asset imported into Vite.
 */
...
/**
 * White/Inverted Mark for Dark Surfaces
 */
export const OrigamiMarkWhite: React.FC<{ size?: number; className?: string }> = ({
  ...
  return (
    <div className={`relative inline-block ${className}`} style={{ width: `${size}px`, height: `${size}px` }}>
      <img
        src={faviconImg}
        alt="MaxWell Logo Mark White"
        width={size}
        height={size}
        className="object-contain brightness-125 filter drop-shadow(0 2px 4px rgba(255,255,255,0.2))"
        style={{ width: `${size}px`, height: `${size}px` }}
      />
    </div>
  );
